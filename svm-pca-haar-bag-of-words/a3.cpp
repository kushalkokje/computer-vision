...
int main(int argc, char **argv)
{
  try {
    ...
    if(algo == "nn")
      ...
      //Second argument is for color
      //true if we want to use color image
      //false if gray scale
      classifier = new SVM(class_list,true);

    }else if(algo == "eigen"){
      //Second argument is number of support vectors
      classifier = new SVM_PCA(class_list,false,100);
    }else if(algo == "haar"){
      //Second argument is number of rectangles
      classifier = new SVM_Haar(class_list,1000);
    }else if(algo == "bow"){
      //Second argument number of clusters
      classifier = new SVM_Kmeans(class_list,20);
    }else if(algo == "deep"){
      classifier = new SVM_CNN(class_list);
    }else
